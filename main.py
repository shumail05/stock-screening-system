...
def main():
    ...
    try:
        ...
        has_fyers = all(st.secrets.get(k) for k in ['FYERS_CLIENT_ID', 'FYERS_SECRET_KEY', 'FYERS_ACCESS_TOKEN'])
        ...
        has_fyers = all(os.environ.get(k) for k in ['FYERS_CLIENT_ID', 'FYERS_SECRET_KEY', 'FYERS_ACCESS_TOKEN'])
        use_mock = not (has_angel or has_fyers)
